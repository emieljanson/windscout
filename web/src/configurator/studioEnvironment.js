...
export function createProductStudioScene(palette = {}) {
  ...
  studio.background = new THREE.Color(palette.background ?? 0xb8bdb9)

  studio.add(
    createReflectionCard('STUDIO_KEY_SOFTBOX', palette.key ?? 0xfffbf3, palette.keyWidth ?? 3.2, 1.9, [-2.4, 1.05, 3.1]),
    createReflectionCard('STUDIO_TOP_SOFTBOX', palette.top ?? 0xf4f7f2, 2.8, 1.25, [0.1, 3.2, 0.15]),
    createReflectionCard('STUDIO_RIM_STRIP', palette.rim ?? 0xd8e7e3, palette.rimWidth ?? 0.55, 2.5, [2.8, 0.35, -1.4]),
    createReflectionCard('STUDIO_DARK_FLAG', palette.dark ?? 0x252b28, 0.68, 2.1, [1.85, 0.15, 2.7]),
    createReflectionCard('STUDIO_BACKDROP', palette.backdrop ?? 0xd6d9d5, 5.5, 3.5, [0, 0.2, -3.7]),
  )

  const floor = createReflectionCard('STUDIO_FLOOR', 0xc7cbc7, 5.5, 5.5, [0, -1.65, 0])
  studio.add(floor)
  return studio
}
