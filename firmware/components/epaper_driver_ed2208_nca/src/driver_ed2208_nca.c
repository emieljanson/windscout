...
static esp_err_t wait_busy(const char *label)
{
    ...
    while (is_busy()) {
        ...
        if (++wait_count > 4000) {  // 40s timeout
            ESP_LOGE(TAG, "[%s] BUSY timeout after 40s", label);
...
esp_err_t epaper_display(uint8_t *image)
{
    ...
    line_buf = heap_caps_malloc(block_w_bytes, MALLOC_CAP_DMA);
    if (!line_buf) {
    ...
    result = spi_device_acquire_bus(spi, portMAX_DELAY);
    if (result != ESP_OK) {
        goto out;
    }
    bus_acquired = true;
    ...
    result = spi_device_transmit(spi, &t_dtm);

    gpio_set_level(g_cfg.pin_dc, 1);  // DATA

    for (uint16_t row = 0; row < EPD_HEIGHT && result == ESP_OK; row++) {
        ...
        result = spi_device_transmit(spi, &t_line);
    }
