...
/**
 * @file zone_info_high.h
 *
 * The data structures in the zone database, with the AtcZoneInfo representing a
 * specific time zone. This is the high resolution storage format equilavent to
 * ZoneInfoHigh.h in the AceTime library. This format provides:
 *  - AT, UNTIL, STDOFF with a 1 second resolution
 *  - SAVE (delta_code) with a 1 second resolution
 *  - year fields using 2-bytes for a range of [-32767,32765] (-32768, 32766,and
 *  32767 are reserved for internal use)
 */

#ifndef ACE_TIME_C_ZONE_INFO_HIGH_H
#define ACE_TIME_C_ZONE_INFO_HIGH_H
