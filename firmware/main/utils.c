    ...
    case HTTP_EVENT_ON_HEADER:
        if (strcasecmp(evt->header_key, "Content-Type") == 0) {
            ...
            if (ctx->thumbnail_url && strlen(evt->header_value) > 0) {
                ...
                ESP_LOGI(TAG, "Thumbnail URL provided");
            }
    ...
    *config_payload_out = NULL;
    ...
    for (int retry = 0; retry < max_retries; retry++) {
        ...
        bool allow_credentials =
            utils_url_allows_credentials(url, config_manager_get_image_url());

        // Add credentials only to the configured HTTPS origin. Redirects are disabled above so
        // the HTTP client cannot forward them to another origin.
        const char *access_token = config_manager_get_access_token();
        if (allow_credentials && access_token && strlen(access_token) > 0) {
        ...
        if (allow_credentials && header_key && strlen(header_key) > 0 && header_value &&
...
static bool fetch_download_thumbnail(const char *thumbnail_url, const char *source_url)
{
    ESP_LOGI(TAG, "Downloading thumbnail");
    ...
    bool allow_credentials = utils_url_allows_credentials(thumbnail_url, source_url);

    // Authenticate only an HTTPS thumbnail on the configured image origin.
    const char *thumb_token = config_manager_get_access_token();
    if (allow_credentials && thumb_token && strlen(thumb_token) > 0) {
    ...
    if (allow_credentials && thumb_hk && strlen(thumb_hk) > 0 && thumb_hv && strlen(thumb_hv) > 0 &&
...
{
    ESP_LOGI(TAG, "Fetching configured image");
    ...
    if (thumbnail_url && strlen(thumbnail_url) > 0) {
        thumbnail_downloaded = fetch_download_thumbnail(thumbnail_url, url);
    }
    free(thumbnail_url);

    if (config_payload && strlen(config_payload) > 0 && !wifi_provisioning_is_provisioned()) {
        fetch_apply_remote_config(config_payload);
    } else if (config_payload && strlen(config_payload) > 0) {
        ESP_LOGW(TAG, "Ignoring remote config outside provisioning mode");
    }
    free(config_payload);
...
esp_err_t trigger_image_rotation(void)
{
    ...
    if (rotation_mode == ROTATION_MODE_URL) {
        ...
        ESP_LOGI(TAG, "URL rotation mode - downloading configured image");
