...
{
    if (!album_manager_is_valid_name(album_name)) {
        return ESP_ERR_INVALID_ARG;
    }
...
{
    if (!album_manager_is_valid_name(album_name) || !path || path_len == 0) {
        return ESP_ERR_INVALID_ARG;
    }

    int written = snprintf(path, path_len, "%s/%s", IMAGE_DIRECTORY, album_name);
