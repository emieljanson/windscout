...
esp_err_t display_manager_init(void)
{
    ...
    s_image_buffer = heap_caps_malloc(EPAPER_LOGICAL_FRAME_BYTES,
                                      MALLOC_CAP_SPIRAM);
    if (!s_image_buffer) {
...
esp_err_t display_manager_begin_rgb_stream(void)
{
    ...
    memset(s_image_buffer, display_white(), EPAPER_LOGICAL_FRAME_BYTES);
...
esp_err_t display_manager_push_rgb_row(int y, const uint8_t *rgb_row, int width)
{
    ...
    if (y >= EPAPER_FRAME_HEIGHT) return ESP_OK;
    GUI_RGBMapFn map_rgb = display_is_grayscale()
                               ? GUI_RGBToGray4
                               : GUI_RGBToSpectra6Logical;
    for (int x = 0; x < width && x < EPAPER_FRAME_WIDTH; ++x) {
        const uint8_t *pixel = &rgb_row[x * 3];
        s_image_buffer[(size_t) y * EPAPER_FRAME_WIDTH + x] =
            map_rgb(pixel[0], pixel[1], pixel[2]);
    }
    return ESP_OK;
}
...
esp_err_t display_manager_push_palette_row(int y, const uint8_t *palette_row, int width)
{
    ...
    if (!s_stream_active) return ESP_ERR_INVALID_STATE;
    if (y >= EPAPER_FRAME_HEIGHT) return ESP_OK;
    const int copy_width = width < EPAPER_FRAME_WIDTH ? width : EPAPER_FRAME_WIDTH;
    memcpy(s_image_buffer + (size_t) y * EPAPER_FRAME_WIDTH,
           palette_row, (size_t) copy_width);
    return ESP_OK;
}
...
esp_err_t display_manager_end_rgb_stream(bool show, const display_publish_t *publish)
{
    ...
    esp_err_t result = show ? epaper_display_logical(
                                  s_image_buffer, EPAPER_LOGICAL_FRAME_BYTES)
                            : ESP_OK;
