...
- The supplied Berkeley Mono file may be used during development. Redistribution rights and the Inter source must be confirmed before public firmware distribution.
...
| Font licensing blocks redistribution | Public firmware cannot include Berkeley Mono | Record the licence before release and substitute a metrically approved font only with design approval. |
...
- **Verification:** Font output is deterministic, width-aware, legally documented, and small enough for safe OTA builds.
...
- Documentation describes provider attribution, licence mode, font licensing, spot/timezone configuration, stale states, and recovery behavior.
