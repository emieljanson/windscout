...
- The standalone site's forecast SVG and visual language are useful references, but only approved assets may enter the public build.
...
- KTD7. **Self-host only approved fonts.** Use Inter and a compatible mono face in the prototype from the project's verified asset set.
...
- Unapproved font files and unlicensed public CAD derivatives are not introduced into a publishable path.
