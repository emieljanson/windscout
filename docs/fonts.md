...
Berkeley Mono was generated from the user-supplied source:
...
not available, `generate_fonts.py` deterministically uses the supplied mono
source for the Inter asset slots and labels that fallback in each generated
file. That fallback is intended for reproducible development only.
