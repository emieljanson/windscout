...
Windscout-owned source code is available under the
[GNU General Public License version 3.0 only](LICENSE). You may use, study,
modify, and redistribute that code under the GPL's terms; distributed modified
versions must remain open under the same license. Pull requests are welcome—see
[`CONTRIBUTING.md`](CONTRIBUTING.md).

Third-party components and assets keep their own terms; see
[`THIRD_PARTY_NOTICES.md`](THIRD_PARTY_NOTICES.md). The software license does
not grant permission to present a modified project as an official Windscout
